...
String KullaniciAdi=request.getParameter("HocaKullaniciAdi");
String AdSoyad=request.getParameter("HocaAdSoyad");
...
while (rs.next()){
	...
	if(idd==AlinanID){
		String dbsifre=rs.getString("HocaSifre");
		if(Md5Sifre.equals(dbsifre)){
			if(Md5YeniSifre.equals(Md5sifretekrar)){
				sayiNesnesi.setHocaSifre(Md5sifretekrar);
				sayiNesnesi.setHocaMail(HocaMail);
				sayiNesnesi.setHocaTarih(Tarih);
				boolean islemSonucu=sayiCrud.hocaguncelle(sayiNesnesi,idd);
				if (islemSonucu==true){
