...
String KullaniciAdi=request.getParameter("YzrKullaniciAdi");
String AdSoyad=request.getParameter("YzrAdSoyad");
String YazarMail=request.getParameter("YzrMail");
...
while (rs.next()){
	...
	if(idd==AlinanID){
		String dbsifre=rs.getString("YzrSifre");
		if(Md5Sifre.equals(dbsifre)){
			if(Md5YeniSifre.equals(Md5sifretekrar)){
				sayiNesnesi.setYzrSifre(Md5sifretekrar);
				sayiNesnesi.setYzrMail(YazarMail);
				sayiNesnesi.setYazarTarih(Tarih);
				boolean islemSonucu=sayiCrud.yazarguncelle(sayiNesnesi,idd);
				if (islemSonucu==true){
