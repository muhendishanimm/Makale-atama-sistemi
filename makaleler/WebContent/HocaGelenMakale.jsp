...
<table class="table table-bordered">
  ...
  while(rs.next()) { 
	  ...
	  if(Durum.equals("Değerlendirme Bekleniyor") || Durum.equals("Değerlendiriliyor"))
  { if(DegerlendirenHoca.equals(HocaAd)){%>
  <tr>
    <td><%=rs.getString("Baslik") %></td>
    <td style="width:30%"><%=icerik%>...</td>
     <td><%=rs.getString("Tarih") %></td>
    <td><%=rs.getString("Yazari") %></td>
    <td><%=rs.getString("Durum") %></td>
    <td><a href='HocaMakaleIncele.jsp?MakaleID=<%=rs.getString("MakaleID")%>' class="btn btn-more"><i class="fa fa-list-alt"></i>İncele</a></td>
  <%}}} %>
 </tr>
</table>
